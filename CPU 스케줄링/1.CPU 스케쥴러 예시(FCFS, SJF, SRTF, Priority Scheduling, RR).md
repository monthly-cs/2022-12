...
가짜 수치를 통해서 표현하면 다음과 같이 분배되는 것이라고 상상하시면 될 것 같습니다.
...
더욱 자세한 예시는 [GeeksOrGeeks - Multilevel Queue (MLQ) CPU Scheduling
](https://www.geeksforgeeks.org/multilevel-queue-mlq-cpu-scheduling/)를 참고해주세요.

### 2.10. [복합] MFQS, Multilevel Feedback Queue Scheduling

다단계 피드백 큐 스케줄링은 MQS, Multilevel Queue Scheduling와 유사하되, 각 프로세스의 대기열 사이의 이도잉 가능하니다.
...
이에 따라서, MFQS는 시간 퀸텀에 대한 프로세스를 실행한 다음 우선 순위를 변경할 수 있습니다.
따라서 프로세스가 **과거 동작에서 학습한 다음 미래 동작을 예측**합니다.
